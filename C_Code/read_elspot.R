...
a = noquote(paste0(elspot_price_2016$X__1[1:(length(elspot_price_2016$X__1)-1)]))
...
# change the date "2016-MM-DD" into "2016MMDDi" where i is the hour
b = noquote(paste0(elspot_price_2017$X__1[1:(length(elspot_price_2017$X__1)-1)]))
b_daily = gsub(b,pattern = "-", replacement = "")
b_monthly = sub("^(\\d{6}).*$", "\\1", b_daily)
b_hourly = paste0(b_daily,year_2017)
# return it in a data frame
elspot_price_2017 = data.frame(date_monthly = b_monthly, date_daily = b_daily, date_hourly = b_hourly, DK1 = elspot_price_2017$DK1[1:(length(elspot_price_2017$X__1)-1)])
